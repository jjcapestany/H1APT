...
public class GraphGenerator {
                ...
                if (inst instanceof INVOKESTATIC invoke) {
                    ...
                    for (Method tm : targetClass.getMethods()) {
                        if (tm.getName().equals(targetMethodName) && tm.getSignature().equals(targetSig)) {
                            ConstantPoolGen targetCpg = new ConstantPoolGen(targetClass.getConstantPool());
                            MethodGen targetMg = new MethodGen(tm, targetClassName, targetCpg);
                            InstructionList targetIl = targetMg.getInstructionList();
                            if (targetIl == null) continue;

                            InstructionHandle[] targetHandles = targetIl.getInstructionHandles();
    ...
    public static void main(String[] a) throws ClassNotFoundException {
        ...
        CFG psetC = gg.createCFG("pset1.C");
        CFG psetD = gg.createCFGWithMethodInvocation("pset1.D");
        System.out.println(psetD.isReachable("main", "pset1.D", "foo", "pset1.D"));
        System.out.println(psetD.isReachable("main", "pset1.D", "bar", "pset1.D"));
    }
}
